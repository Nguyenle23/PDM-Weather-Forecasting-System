...
class Update_User_user extends Add_wData implements ActionListener{
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        try{
            conn con = new conn();
            //sql statement to display all the data of the current user in the login table
            String str = "select * from login where username = '"+login.getUsername()+"'";
        ...
        if(ae.getSource()==b && i==1){
            try{
                conn con = new conn();
                //sql statement to update the data of the current user
                String str = "update login set  upassword = '"+t2.getText()+"', Email = '"+t4.getText()+"' where username ='"+login.getUsername()+"';";
